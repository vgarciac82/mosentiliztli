...
# Request-scoped Redmine client (overrides the default service credentials)
_request_redmine_client: ContextVar[Optional[Redmine]] = ContextVar(
    "request_redmine_client", default=None
)


def _build_requests_config() -> Dict[str, Any]:
    """Build shared requests configuration for Redmine clients."""
    requests_config: Dict[str, Any] = {}
...
def _create_redmine_client(
    *,
    api_key: Optional[str] = None,
    username: Optional[str] = None,
    password: Optional[str] = None,
) -> Optional[Redmine]:
    """Create a Redmine client using API key or username/password auth."""
    effective_api_key = (api_key or "").strip() or None
    ...
    if not (
        effective_api_key
        or (effective_username is not None and effective_password is not None)
    ):
        return None
    ...
    if effective_api_key:
        if requests_config:
            return Redmine(REDMINE_URL, key=effective_api_key, requests=requests_config)
        return Redmine(REDMINE_URL, key=effective_api_key)
...
def _create_request_scoped_redmine_client(
    headers: Mapping[str, str],
) -> Optional[Redmine]:
    """Create a request-scoped Redmine client from forwarded HTTP headers."""
    header_api_key = (headers.get("x-redmine-api-key") or "").strip() or None
    header_username = (headers.get("x-redmine-username") or "").strip() or None
    header_password = (headers.get("x-redmine-password") or "").strip() or None

    if not (header_api_key or header_username or header_password):
        return None

    return _create_redmine_client(
...
def _get_active_redmine_client() -> Optional[Redmine]:
    """Return the request-scoped Redmine client, falling back to the default."""
    request_client = _request_redmine_client.get()
    if request_client is not None:
        return request_client
    return _default_redmine_client


class _RedmineClientProxy:
    """Resolve the active Redmine client lazily per request."""

    def __bool__(self) -> bool:
        return _get_active_redmine_client() is not None

    def __getattr__(self, attr: str) -> Any:
        client = _get_active_redmine_client()
        if client is None:
            raise AttributeError(
                "Redmine client is not initialized. "
                "Configure default credentials or send X-Redmine-API-Key."
            )
...
# For simplicity, we'll initialize the default service client globally and expose
# a proxy that can swap to request-scoped credentials when headers are provided.
_default_redmine_client = None
if REDMINE_URL and (REDMINE_API_KEY or (REDMINE_USERNAME and REDMINE_PASSWORD)):
    try:
        _default_redmine_client = _create_redmine_client(
